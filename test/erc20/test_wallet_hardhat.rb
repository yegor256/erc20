...
class TestWalletHardhat < ERC20::Test
  ...
  def test_checks_balance_in_threads
    WebMock.enable_net_connect!
    on_hardhat do |wallet|
  ...
  def test_accepts_payments_after_disconnect
  ...
  def test_accepts_many_payments
    WebMock.enable_net_connect!
    walter = Eth::Key.new(priv: WALTER).address.to_s.downcase
    on_hardhat do |wallet|
      active = []
  ...
  def test_accepts_payments_with_failures
    WebMock.enable_net_connect!
    walter = Eth::Key.new(priv: WALTER).address.to_s.downcase
    on_hardhat do |wallet|
      active = []
  ...
  def test_accepts_payments_on_changing_addresses
    WebMock.enable_net_connect!
    walter = Eth::Key.new(priv: WALTER).address.to_s.downcase
    jeff = Eth::Key.new(priv: JEFF).address.to_s.downcase
  ...
  def test_accepts_payments_via_proxy
    WebMock.enable_net_connect!
    via_proxy do |proxy|
      walter = Eth::Key.new(priv: WALTER).address.to_s.downcase
      jeff = Eth::Key.new(priv: JEFF).address.to_s.downcase
